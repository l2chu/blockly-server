...
Robot.drawRotated = function(degrees){
  ...
  Robot.ctxDisplay.translate(-(Robot.x),-(Robot.y));
  Robot.ctxDisplay.drawImage(Robot.image,Robot.x-(Robot.widthPic/2),Robot.y-(Robot.heightPic/2));
...
Robot.animate = function() {
  ...
  switch (tuple[0]) {
    ...
    case 'LF':  // LED off
      ...
      break;
    case 'GW':  // GPIO write
      socket.emit('gpio', {type: 'write GPIO', command: tuple[1]});
      break;
    case 'GR':  // GPIO read
      var gpioBlock;
      socket.emit('gpio', {type: 'read GPIO', command: 'reading'});
      socket.on('gpio-read', function(data){
        gpioBlock = Blockly.mainWorkspace.getBlockById(tuple[1]);
        gpioBlock.setFieldValue(JSON.stringify(data), 'VALUE');
      });
      break;
    case 'MO':  // Motor on
...
Robot.changeGPIO = function(num, id) {
  Blockly.Apps.log.push(['GW', num, id]);
};
 ...
 * @param {number} id The block ID.
 */
Robot.readGPIO = function(id) {
  Blockly.Apps.log.push(['GR', id]);
};
